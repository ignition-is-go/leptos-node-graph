...
use crate::widgets::{NumberInput, Select, options_from};
...
pub fn build_node_registry() -> NodeTypeRegistry {
    ...
    reg.register_port_type_slot(super::DemoPort::Float, PortDirection::Input, |label| {
    ...
    for item in catalog {
        ...
        match type_id.as_str() {
                ...
                reg.register(
                    NodeTypeBuilder::<super::DemoPort>::new(item)
                        .body(|_node_id| {
                            ...
                            let num_in = Signal::derive(move || {
                                num_in_str.get().parse::<usize>().unwrap_or(2)
                            });
                            let num_out = Signal::derive(move || {
                                num_out_str.get().parse::<usize>().unwrap_or(1)
                            });
                            provide_context(CustomPortCounts {
                                inputs: num_in,
                                outputs: num_out,
                            });
                            let count_opts: Vec<(String, String)> =
                                (0..=8).map(|n| (n.to_string(), n.to_string())).collect();
            ...
            _ => {
                reg.register(NodeTypeBuilder::<super::DemoPort>::new(item).build());
            }
        }
    }
